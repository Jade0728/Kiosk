...
public class EwhaSchoolFood extends JFrame {
	...
	public EwhaSchoolFood() {
		...
		pNorth.setFont(font); 
		pNorth.add(highbar);//North Panel
		
		String menu [] = {"떡볶이", "라면", "우동", "김밥", "쫄면", "냉면", "콜라", "사이다"};
		int price[] = {3500, 2000, 3000, 2500, 3000, 3500, 1500, 1500};
        ...
        //메뉴 이름, 메뉴 위치 정하기
        JLabel menuname []= new JLabel [8];
        for(int i=0;i<8;i++) {
        	menuname[i]= new JLabel();
        	menuname[i].setText(menu[i]);
        	menuname[i].setBounds(60 + i * 150, 40, 70, 20); 
        ...
        for (int i = 0; i < menu.length; i++) {
			...
			menuname[i].setBounds(bt[i].getX() + 30, bt[i].getY() + -20, 100, 20);
        ...
        for (int i = 0; i < menu.length; i++) {
         ...
         // Plus button event
            ...
            ok[i].addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                	if (menu[j].length() == 3) {
                		 ta. append("  " + menu[j] + "        " + price[j] + "        " + count[j] + "         " + price[j]*count[j]
                                 + "원" + "\n");
                	} else {
                		ta. append("  " + menu[j] + "          " + price[j] + "        " + count[j] + "         " + price[j]*count[j]
                                + "원" + "\n");
